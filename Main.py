...
def loadwallet():
    ...
    getstakecall = CallBuilder().from_("hx0000000000000000000000000000000000000001")\
                    .to(GOV_CONTRACT)\
                    .method("getStake")\
                    .params({"address": adress})\
                    .build()
    getstakeresult = icon_service.call(getstakecall)
    stake = getstakeresult.get('stake')

    decstake = literal_eval(stake)
    convstake = decstake / EXA
    strstake = str(convstake)
    ...
    table.add_column("Staked ICX:", justify="right")
    table.add_row(
    adress, strbalance, strusdbalance, strstake
    )
    console.print(table)
...
def claimiscore():
    ...
    deciscore = literal_eval(data2)
    convdeciscore = deciscore / EXA
    strconviscore = str(convdeciscore)

    print("iScore Claimed: " + strconviscore)
    menu()
...
def queryiscore():
    ...
    deciscore = literal_eval(iscore)
    convdeciscore = deciscore / EXA
    strconviscore = str(convdeciscore)

    decestimatedicx = literal_eval(estimatedicx)
    convestimatedicx = decestimatedicx / EXA
    strconvestimatedicx = str(convestimatedicx)
    ...
    table.add_row(
    strconvestimatedicx, strconviscore
    ) 
    console.print(table)
